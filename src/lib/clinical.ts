...
export const LABS: LabDef[] = [
  {
    ...
    label: "Hemoglobina",
    unit: "g/dL",
    ref: { low: 13, high: 17, veryLow: 8, veryHigh: 19 },
    refF: { low: 12, high: 16, veryLow: 8, veryHigh: 18 },
    risingIs: "good",
  },
  {
    code: "Ht",
    label: "Hematócrito",
    unit: "%",
    ref: { low: 40, high: 52, veryLow: 25, veryHigh: 58 },
    refF: { low: 36, high: 47, veryLow: 25, veryHigh: 55 },
    risingIs: "good",
  },
  {
    code: "Leuco",
    label: "Leucócitos",
    unit: "/mm³",
    ref: { low: 4000, high: 11000, veryLow: 2000, veryHigh: 20000 },
    risingIs: "bad",
  },
  {
    code: "Plaq",
    label: "Plaquetas",
    unit: "/mm³",
    ref: { low: 150000, high: 450000, veryLow: 50000, veryHigh: 800000 },
    risingIs: "good",
  },
  {
    code: "PCR",
    label: "PCR",
    unit: "mg/dL",
    ref: { low: 0, high: 0.5, veryHigh: 10 },
    risingIs: "bad",
  },
  {
    code: "Lactato",
  ...
  },
  {
    ...
    label: "Creatinina",
    unit: "mg/dL",
    ref: { low: 0.6, high: 1.3, veryHigh: 3 },
    risingIs: "bad",
  },
  {
    code: "Ureia",
    label: "Ureia",
    unit: "mg/dL",
    ref: { low: 15, high: 45, veryHigh: 100 },
    risingIs: "bad",
  },
  {
    code: "Na",
    label: "Sódio",
    unit: "mEq/L",
    ref: { low: 135, high: 145, veryLow: 125, veryHigh: 155 },
    risingIs: "neutral",
  },
  {
    code: "K",
    label: "Potássio",
    unit: "mEq/L",
    ref: { low: 3.5, high: 5.0, veryLow: 2.8, veryHigh: 6.0 },
    risingIs: "neutral",
  },
  ...
  },
  {
    ...
    label: "Glicemia",
    unit: "mg/dL",
    ref: { low: 70, high: 180, veryLow: 50, veryHigh: 250 },
    risingIs: "neutral",
  },
  {
    code: "pH",
    label: "pH arterial",
    unit: "",
    ref: { low: 7.35, high: 7.45, veryLow: 7.2, veryHigh: 7.55 },
    risingIs: "neutral",
  },
  {
    code: "PaO2",
    label: "PaO₂",
    unit: "mmHg",
    ref: { low: 80, high: 100, veryLow: 60 },
    risingIs: "good",
  },
  {
    code: "PaCO2",
    label: "PaCO₂",
    unit: "mmHg",
    ref: { low: 35, high: 45, veryHigh: 60 },
    risingIs: "neutral",
  },
  {
    code: "HCO3",
  ...
  },
  {
    ...
    label: "SatO₂",
    unit: "%",
    ref: { low: 92, high: 100, veryLow: 85 },
    risingIs: "good",
  },
  {
    code: "BE",
    label: "BE (Base Excess)",
    unit: "mEq/L",
    ref: { low: -2, high: 2, veryLow: -6, veryHigh: 6 },
    risingIs: "neutral",
  },
  {
    code: "PF",
    ...
    unit: "",
    ref: { low: 300, high: 500, veryLow: 100 },
    risingIs: "good",
  },

  {
    ...
    label: "INR",
    unit: "",
    ref: { low: 0.9, high: 1.2, veryHigh: 3 },
    risingIs: "bad",
  },
  {
    code: "Trop",
    label: "Troponina",
    unit: "ng/mL",
    ref: { low: 0, high: 0.04, veryHigh: 1 },
    risingIs: "bad",
  },
  {
    code: "BNP",
    label: "BNP",
    unit: "pg/mL",
    ref: { low: 0, high: 100, veryHigh: 400 },
    risingIs: "bad",
  },
  {
    code: "Bilir",
    ...
    unit: "mg/dL",
    ref: { low: 0.2, high: 1.2, veryHigh: 5 },
    risingIs: "bad",
  },
  {
    code: "PIC",
...
export function bucketBadge(b: LabBucket) {
  switch (b) {
    case "very_low":
      return { icon: "", className: "text-clinical-critical", label: "Muito baixo" };
    case "low":
      return { icon: "", className: "text-clinical-attention", label: "Baixo" };
    case "normal":
      return { icon: "", className: "text-clinical-stable", label: "Normal" };
    case "high":
      return { icon: "", className: "text-clinical-device", label: "Elevado" };
    case "very_high":
      return { icon: "", className: "text-clinical-critical", label: "Muito elevado" };
  }
}
...
export function trendBadge(t: TrendDirection) {
  switch (t) {
    case "improving":
      return { icon: "", className: "text-clinical-stable", label: "Melhorando" };
    case "worsening":
      return { icon: "", className: "text-clinical-critical", label: "Piorando" };
    case "flat":
      return { icon: "", className: "text-clinical-neutral", label: "Estável" };
  }
}
...
export function examInsight(e: ExamRow, sex: "M" | "F" = "M") {
  ...
  const series =
    value !== null && (!history.length || history[0] !== value) ? [value, ...history] : history;
...
export function trendArrow(movement: "up" | "down" | "flat", trend: TrendDirection) {
...
export const drugByName = (n: string) =>
  DRUGS.find((d) => d.name.toLowerCase() === n.toLowerCase());
...
export function generateEstadoAtual(p: Patient): EstadoLine[] {
  ...
  const atbs = activeMeds.filter((m) =>
    /cef|mero|pip|vanco|cipro|levo|ampi|metro|linezo|polimi|tigec|ertap|amik|genta|sulb/i.test(
      m.name,
    ),
  );
  ...
  lines.push({
    ...
    text:
      s.diurese < 0.5
        ? `Oligúria (${s.diurese.toFixed(1)} ml/kg/h)`
        : `Diurese ${s.diurese.toFixed(1)} ml/kg/h`,
...
export function computeBMI(weightKg?: number, heightCm?: number): BMIInfo | null {
  ...
  if (v < 18.5) {
    ...
    label = "Obesidade I";
    className = "text-clinical-device";
  } else if (v < 40) {
    label = "Obesidade II";
    className = "text-clinical-device";
  } else {
...
export function computeCrCl(
  p: Patient,
): { value: number; creat: number; ageYears: number; weightKg: number } | null {
...
export function detectAntibiotic(name: string): boolean {
  return /cef|mero|pip|vanco|cipro|levo|ampi|metro|linezo|polimi|tigec|ertap|amik|genta|clinda|sulb|claritro|azitro/i.test(
    name,
  );
}
...
export function antibioticProgress(med: Medication): AtbProgress | null {
  ...
  return {
    ...
    plannedDoses: med.plannedDoses,
  };
}

export function atbAlertBadge(
  a: AtbAlert,
): { icon: string; label: string; className: string } | null {
  switch (a) {
    case "ending_48h":
      return { icon: "", label: "Termina em 48h", className: "text-clinical-attention" };
    case "ending_24h":
      return { icon: "", label: "Termina em 24h", className: "text-clinical-device" };
    case "last_dose":
      return { icon: "", label: "Última dose", className: "text-clinical-critical" };
    case "overdue":
      return { icon: "", label: "Curso encerrado", className: "text-clinical-critical" };
    case "ok":
      return null;
  }
}
...
export const PUMP_CATEGORY_META: Record<
  PumpCategory,
...
> = {
  vasoativa: {
    ...
    label: "Vasoativa",
    className: "text-clinical-critical",
    dotClass: "bg-clinical-critical",
  },
  sedativo: {
    icon: "",
    label: "Sedativo",
    className: "text-clinical-neuro",
    dotClass: "bg-clinical-neuro",
  },
  analgesico: {
    icon: "",
    label: "Analgésico",
  ...
  antimicrobiano: {
    ...
    className: "text-clinical-stable",
    dotClass: "bg-clinical-stable",
  },
  insulina: {
    icon: "",
    label: "Insulina",
    className: "text-clinical-attention",
    dotClass: "bg-clinical-attention",
  },
  hidratacao: {
    icon: "",
    label: "Hidratação",
    className: "text-clinical-neutral",
    dotClass: "bg-clinical-neutral",
  },
  outro: {
    icon: "",
    label: "Outro",
...
export const DILUTION_LIBRARY = DRUGS.reduce<
  Record<
    string,
    Record<
      "Geral" | "Neuro" | "Cardio",
      { amountMg: number; volumeMl: number; solvent: SolventType; label: string }
    >
  >
>((acc, d) => {
  acc[d.name] = {
    Geral: {
      amountMg: d.protocols.Geral.ampMg,
      volumeMl: d.protocols.Geral.diluentMl,
      solvent: "SF 0,9%",
      label: d.protocols.Geral.name,
    },
    Neuro: {
      amountMg: d.protocols.Neuro.ampMg,
      volumeMl: d.protocols.Neuro.diluentMl,
      solvent: "SF 0,9%",
      label: d.protocols.Neuro.name,
    },
    Cardio: {
      amountMg: d.protocols.Cardio.ampMg,
      volumeMl: d.protocols.Cardio.diluentMl,
      solvent: "SG 5%",
      label: d.protocols.Cardio.name,
    },
  };
  return acc;
}, {});
...
export function computeBagRemaining(
  pump: PumpInfusion,
  ...
  return {
    ...
    remainingMin,
    endsAt,
    percent: Math.min(100, (consumed / bag) * 100),
    alert,
  };
}

export function bagAlertBadge(a: BagAlert): {
  icon: string;
  label: string;
  className: string;
  borderClass: string;
  ...
  switch (a) {
    case "low_2h":
      return {
        icon: "",
        label: "Restam 2 horas",
        className: "text-clinical-attention",
        borderClass: "border-clinical-attention/60",
      };
    case "low_1h":
      return {
        icon: "",
        label: "Resta 1 hora",
        className: "text-clinical-device",
        borderClass: "border-clinical-device/60",
      };
    case "low_30min":
      return {
        icon: "",
        label: "Restam 30 min",
        className: "text-clinical-critical",
        borderClass: "border-clinical-critical/70",
      };
    case "empty":
      return {
        icon: "",
        label: "Bolsa finalizada",
        className: "text-clinical-critical",
        borderClass: "border-clinical-critical",
      };
    case "ok":
      return {
        icon: "",
...
export const MEDICATION_CLASS_META: Record<
  MedicationClass,
  {
    label: string;
    short: string;
...
> = {
  antibiotic: {
    ...
    icon: "",
    className: "text-clinical-attention",
    bgClass: "bg-clinical-attention/10",
    borderClass: "border-clinical-attention/40",
  },
  pump: {
    label: "Bomba",
    short: "Bomba",
    icon: "",
    className: "text-clinical-critical",
    bgClass: "bg-clinical-critical/10",
    borderClass: "border-clinical-critical/40",
  },
  hydration: {
    label: "Hidratação",
    short: "Hidrat.",
    icon: "",
    className: "text-clinical-resp",
    bgClass: "bg-clinical-resp/10",
    borderClass: "border-clinical-resp/40",
  },
  iv: {
    label: "Venoso",
    short: "EV",
  ...
  im: {
    ...
    icon: "",
    className: "text-clinical-device",
    bgClass: "bg-clinical-device/10",
    borderClass: "border-clinical-device/40",
  },
  sc: {
    label: "Subcutâneo",
    short: "SC",
    icon: "",
    className: "text-clinical-stable",
    bgClass: "bg-clinical-stable/10",
    borderClass: "border-clinical-stable/40",
  },
  oral: {
    label: "Oral/Enteral",
    short: "VO/Ent.",
  ...
  topical: {
    ...
    icon: "",
    className: "text-clinical-neutral",
    bgClass: "bg-clinical-neutral/10",
    borderClass: "border-clinical-neutral/40",
  },
};
...
export const BRISTOL: {
...
}[] = [
  {
    value: 1,
    label: "Tipo 1",
    hint: "Bolinhas duras separadas — constipação grave",
    color: "#6b4a2b",
    className: "text-clinical-critical",
  },
  {
    value: 2,
    label: "Tipo 2",
    hint: "Salsicha grumosa — constipação",
    color: "#8b5a2b",
    className: "text-clinical-device",
  },
  {
    value: 3,
    label: "Tipo 3",
    hint: "Salsicha com rachaduras — normal (firme)",
    color: "#a67b3e",
    className: "text-clinical-attention",
  },
  {
  ...
  },
  {
    value: 5,
    label: "Tipo 5",
    hint: "Pedaços macios com bordas nítidas",
    color: "#a88551",
    className: "text-clinical-attention",
  },
  {
    value: 6,
    label: "Tipo 6",
    hint: "Fragmentos moles / pastoso — diarreia leve",
    color: "#b28e5d",
    className: "text-clinical-device",
  },
  {
    value: 7,
    label: "Tipo 7",
    hint: "Líquido sem sólidos — diarreia grave",
    color: "#c8a074",
    className: "text-clinical-critical",
  },
];
...
export function computeFluidBalance(fb?: {
  intake?: FluidEntry[];
  output?: FluidEntry[];
  drains?: DrainEntry[];
...
export const CONDUCT_SYSTEM_META: Record<
  ConductSystem,
  {
    label: string;
    short: string;
...
> = {
  dieta: {
    ...
    className: "text-clinical-nutri",
    bgClass: "bg-clinical-nutri/10",
    borderClass: "border-clinical-nutri/50",
  },
  fono: {
    label: "Fonoterapia",
    short: "Fono",
    icon: "",
    className: "text-clinical-neuro",
    bgClass: "bg-clinical-neuro/10",
    borderClass: "border-clinical-neuro/50",
  },
  gi: {
    label: "Digestivo",
    short: "Digest",
  ...
  cardio: {
    ...
    className: "text-clinical-critical",
    bgClass: "bg-clinical-critical/10",
    borderClass: "border-clinical-critical/50",
  },
  resp: {
    label: "Respiratório",
    short: "Resp",
    ...
    className: "text-clinical-resp",
    bgClass: "bg-clinical-resp/10",
    borderClass: "border-clinical-resp/50",
  },
  renal: {
    ...
    className: "text-clinical-attention",
    bgClass: "bg-clinical-attention/10",
    borderClass: "border-clinical-attention/50",
  },
  infec: {
    label: "Sepse/Infecção",
    short: "Infec",
    icon: "",
    className: "text-clinical-device",
    bgClass: "bg-clinical-device/10",
    borderClass: "border-clinical-device/50",
  },
  hemato: {
    label: "Hematológico",
    short: "Hemato",
...
export const ANNOTATION_COLOR_META: Record<
  AnnotationColor,
  { label: string; textClass: string; swatch: string }
> = {
  default: { label: "Preto", textClass: "text-ink", swatch: "#000000" },
  green: { label: "Verde limão", textClass: "text-lime-600 dark:text-lime-400", swatch: "#84cc16" },
  yellow: {
    label: "Amarelo",
...
export function directiveLabel(
  value: "allow" | "refuse" | "unknown" | undefined,
  topic: string,
): string {
