...
function para(
  ...
  s: string,
  yIn: number,
  opts?: { size?: number; color?: readonly number[] },
) {
...
function box(
  ...
  w: number,
  h: number,
  fill: readonly number[] = BG,
  stroke: readonly number[] = LINE,
) {
...
export function buildFamilyReport(patient: Patient): jsPDF {
  ...
  txt(doc, "Um relato acolhedor sobre o cuidado que vem sendo dedicado ao seu familiar", M, 17.5, {
    size: 9.5,
    color: [214, 222, 228],
  });
  ...
  txt(
    doc,
    `Leito ${patient.bed} · ${patient.age} anos · Equipe ${patient.team} · Médico(a) ${patient.attending}`,
    M + 4,
    y + 13.5,
    { size: 9, color: SOFT, maxWidth: CW - 60 },
  );
  doc.setFillColor(sev.color[0], sev.color[1], sev.color[2]);
  doc.roundedRect(W - M - 54, y + 4, 50, 12, 2, 2, "F");
  txt(doc, sev.label, W - M - 29, y + 8.6, {
    size: 7.5,
    bold: true,
    color: [255, 255, 255],
    align: "center",
  ...
  const current = patient.diagnoses.filter(
    (d) => d.category !== "previous" && d.category !== "inactive",
  );
  if (current.length) {
    y = para(
      doc,
      `O motivo que trouxe o paciente até nós está relacionado a ${current
        .slice(0, 4)
        .map((d) => d.label.toLowerCase())
        .join(
          ", ",
        )}. Compreender essa condição é o primeiro passo do tratamento, e é sobre ela que concentramos nossos esforços diários.`,
      y,
    );
  }
  ...
  if (previous.length) {
    y = para(
      doc,
      `Também consideramos, em todas as decisões, as condições de saúde que já existiam antes desta internação — como ${previous
        .slice(0, 5)
        .map((d) => d.label.toLowerCase())
        .join(
          ", ",
        )} — porque o cuidado em UTI é sempre pensado para a pessoa como um todo, e não apenas para o problema do momento.`,
      y,
    );
  }
  ...
  if (meds.length) {
    y = para(
      doc,
      `Os medicamentos em uso foram escolhidos criteriosamente e são reavaliados todos os dias. Entre eles estão ${meds
        .slice(0, 5)
        .map((m) => m.name.toLowerCase())
        .join(
          ", ",
        )}${meds.length > 5 ? ", entre outros" : ""}. Cada medicação tem uma finalidade precisa: controlar a infecção, manter a pressão e os batimentos estáveis, aliviar a dor, garantir o conforto e proteger os órgãos enquanto o corpo se recupera.`,
      y,
    );
  }
  ...
  if (patient.state.vent || patient.state.dva || patient.state.dieta) {
    ...
    if (patient.state.vent)
      supports.push(
        `a respiração está sendo auxiliada (${patient.state.vent}), para que os pulmões descansem e se recuperem`,
      );
    if (patient.state.dva)
      supports.push(
        `a pressão do sangue recebe apoio de medicação (${patient.state.dva}), garantindo que todos os órgãos sejam bem irrigados`,
      );
    if (patient.state.dieta)
      supports.push(
        `a alimentação é feita por ${patient.state.dieta}, assegurando a nutrição necessária para a cicatrização e a força muscular`,
      );
    y = para(
      doc,
  ...
  if (patient.conducts.length) {
    const sysLabels = Array.from(
      new Set(
        patient.conducts
          .map((c) => (c.system ? CONDUCT_SYSTEM_META[c.system]?.label : undefined))
          .filter(Boolean),
      ),
    ) as string[];
    y = para(
      doc,
      `O plano de cuidados do dia é organizado em anotações por sistema do organismo${
        sysLabels.length
          ? ` — hoje com atenção especial a ${sysLabels
              .slice(0, 5)
              .map((s) => s.toLowerCase())
              .join(", ")}`
          : ""
      }. Essa rotina disciplinada, repetida dia após dia, é o que transforma pequenas melhoras em uma recuperação sólida.`,
      y,
    );
  }
  ...
  if (patient.goals?.length) {
    ...
    if (met.length)
      parts.push(
        `já foram alcançadas metas importantes, como ${met
          .slice(0, 3)
    ...
    if (open.length)
      parts.push(
        `seguimos trabalhando em direção a ${open
          .slice(0, 3)
          .map((g) => g.text.toLowerCase())
          .join("; ")}`,
      );
  ...
  txt(
    doc,
    "Este documento é um resumo em linguagem simples e não substitui a conversa com o médico responsável. Traga suas dúvidas na próxima visita: estaremos à disposição para conversar com calma e transparência.",
    M + 4,
    y + 13,
    { size: 9.5, color: INK, maxWidth: CW - 8 },
  );
  if (patient.legalRepresentative?.name) {
    txt(
      doc,
      `Contato de referência da família: ${patient.legalRepresentative.name}${patient.legalRepresentative.relation ? ` (${patient.legalRepresentative.relation})` : ""}${patient.legalRepresentative.phone ? ` · ${patient.legalRepresentative.phone}` : ""}`,
      M + 4,
      y + 26,
      { size: 9, color: SOFT, maxWidth: CW - 8 },
    );
  }
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    txt(doc, `${patient.name} · Leito ${patient.bed} · Documento para a família`, M, 289, {
      size: 8,
      color: SOFT,
    });
