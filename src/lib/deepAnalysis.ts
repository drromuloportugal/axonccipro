...
function series(label: string, readings?: { value?: number; min?: number; at?: string }[]) {
  ...
  const items = readings
    ...
    .map(
      (r) =>
        `${r.at ? date(r.at) : "s/ data"}: ${fmt(r.value)}${r.min !== undefined ? ` (mín ${r.min})` : ""}`,
    )
    .join(" | ");
  return `- ${label}: ${items}\n`;
}
...
export function buildPassometroContext(p: Patient): string {
  ...
  const diagnosticos = (p.diagnoses ?? [])
    .map(
      (d) =>
        `- [${fmt(d.category)}] ${d.date ? date(d.date) : "s/ data"} — ${d.label}${d.detail ? ` (${d.detail})` : ""}`,
    )
    .join("\n");
    ...
    (p.pastMedications ?? []).length
      ? "Medicações prévias / uso habitual:\n" +
        (p.pastMedications ?? [])
          .map(
            (m) =>
              `- ${m.name}${m.dose ? ` ${m.dose}` : ""}${m.route ? ` ${m.route}` : ""}${m.freq ? ` ${m.freq}` : ""}${m.status ? ` · ${m.status}` : ""}${m.reason ? ` · indicação: ${m.reason}` : ""}${m.period ? ` · período: ${m.period}` : ""}`,
          )
          .join("\n")
  ...
  const procedimentos = (p.procedures ?? [])
    ...
    .map(
      (e) =>
        `- ${e.date ? date(e.date) : "s/ data"} — ${e.label}${e.detail ? `: ${e.detail}` : ""}`,
    )
    .join("\n");
  ...
  const imagem = (p.imaging ?? [])
    ...
    .map(
      (i) =>
        `- ${date(i.performedAt)} — ${i.modality} de ${i.region} · status ${fmt(i.status)} · conclusão ${fmt(i.conclusion)}${i.summary ? `\n    achado: ${i.summary}` : ""}${i.reportedBy ? ` · laudo por ${i.reportedBy}` : ""}`,
    )
    .join("\n");
  ...
  const hemo = (p.hemotransfusions ?? [])
    .map(
      (h) =>
        `- ${date(h.date)} — ${h.component}${h.volume ? ` · ${h.volume}` : ""}${h.note ? ` · ${h.note}` : ""}`,
    )
    .join("\n");
  ...
  const dispositivos = (p.devices ?? [])
    .map(
      (d) =>
        `- ${d.typeCode} (${d.category})${d.site ? ` · ${d.site}` : ""}${d.side ? ` ${d.side}` : ""}${d.size ? ` · ${d.size}` : ""} · inserido em ${date(d.insertedAt)}${d.removedAt ? ` · retirado em ${date(d.removedAt)}` : " · EM USO"}${d.indication ? ` · indicação: ${d.indication}` : ""}${d.notes ? ` · ${d.notes}` : ""}`,
    )
    .join("\n");
  ...
  const infeccoes = (p.infections ?? [])
    .map(
      (f) =>
        `- foco ${f.site} · ${f.status}${f.unstable ? " · INSTÁVEL" : ""} · início ${date(f.startedAt)}${f.resolvedAt ? ` · resolvido ${date(f.resolvedAt)}` : ""}${(f.antimicrobials ?? []).length ? ` · antimicrobianos: ${(f.antimicrobials ?? []).join(", ")}` : ""}${f.notes ? ` · ${f.notes}` : ""}`,
    )
    .join("\n");
  ...
  const culturas = (p.cultures ?? [])
    ...
    .map(
      (c) =>
        `- ${date(c.collectedAt)} — ${c.source}${c.collectionSite ? ` (${c.collectionSite})` : ""}: ${fmt(c.result)}${c.organism ? ` · ${c.organism}` : ""}${c.resistanceProfile ? ` · perfil ${c.resistanceProfile}` : ""}${(c.resistances ?? []).length ? ` · R: ${(c.resistances ?? []).join(", ")}` : ""}${(c.sensitivities ?? []).length ? ` · S: ${(c.sensitivities ?? []).join(", ")}` : ""}${c.notes ? ` · ${c.notes}` : ""}`,
    )
    .join("\n");
  ...
  const lpp = (p.lpp ?? [])
    .map(
      (l) =>
        `- ${l.siteLabel ?? l.site} · estágio ${l.stage}${l.side ? ` · ${l.side}` : ""} · identificada em ${date(l.identifiedAt)}${l.resolvedAt ? ` · resolvida ${date(l.resolvedAt)}` : ""}${(l.evolutions ?? []).length ? ` · evoluções: ${(l.evolutions ?? []).map((e) => `${date(e.at)} estágio ${e.stage}${e.notes ? ` (${e.notes})` : ""}`).join("; ")}` : ""}`,
    )
    .join("\n");
  ...
  const intub = (p.intubations ?? [])
    .map(
      (i) =>
        `- ${date(i.createdAt)} — IOT modo ${i.mode} · ${i.status}${i.procedure?.cormack ? ` · Cormack ${i.procedure.cormack}` : ""}${i.procedure?.attempt ? ` · ${i.procedure.attempt} tentativa(s)` : ""}${(i.complications ?? []).length ? ` · complicações: ${(i.complications ?? []).join(", ")}` : ""}${i.postVent ? ` · pós-IOT: ${JSON.stringify(i.postVent)}` : ""}${i.notes ? ` · ${i.notes}` : ""}`,
    )
    .join("\n");
  ...
  const seriados =
    ...
    (s.customSeries ?? [])
      .map((c) => series(`${c.label}${c.unit ? ` (${c.unit})` : ""}`, c.readings))
      .join("");
  ...
  const drains = (s.fluidBalance?.drains ?? [])
    .map(
      (d) =>
        `- dreno ${d.name}${d.site ? ` (${d.site})` : ""}: ${d.volumeMl} mL${d.aspect ? ` · ${d.aspect}` : ""}${d.at ? ` · ${date(d.at)}` : ""}`,
    )
    .join("\n");
  const derivations = (s.fluidBalance?.derivations ?? [])
    .map(
      (d) =>
        `- derivação ${d.name}${d.site ? ` (${d.site})` : ""}: ${d.volumeMl} mL${d.aspect ? ` · ${d.aspect}` : ""}${d.at ? ` · ${date(d.at)}` : ""}`,
    )
    .join("\n");
