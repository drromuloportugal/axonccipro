...
export const DEVICE_CATEGORIES: { code: DeviceCategory; label: string; icon: string }[] = [
  ...
  { code: "arterial",          label: "Cateter arterial",         icon: "❤️" },
  ...
  { code: "advanced",          label: "Terapias avançadas",       icon: "⚙️" },
...
export const DEVICE_TYPES: DeviceTypeDef[] = [
  ...
  // ── Cateteres arteriais ──────────────────────────────────────────────────
  {
    code: "PAI_RAD", label: "PAM invasiva — radial", icon: "❤️", category: "arterial",
    sites: ["Radial D", "Radial E"], recommendedMaxDays: 7, riskWeight: 2,
  },
  {
    code: "PAI_FEM", label: "PAM invasiva — femoral", icon: "❤️", category: "arterial",
    sites: ["Femoral D", "Femoral E"], recommendedMaxDays: 5, riskWeight: 3,
  },
  {
    code: "PAI_BRA", label: "PAM invasiva — braquial", icon: "❤️", category: "arterial",
    sites: ["Braquial D", "Braquial E"], recommendedMaxDays: 5, riskWeight: 3,
  },
  {
    code: "PAI_PED", label: "PAM invasiva — pediosa", icon: "❤️", category: "arterial",
  ...
  // ── Terapias avançadas ──────────────────────────────────────────────────
  { code: "ECMO_VV", label: "ECMO veno-venosa",  icon: "⚙️", category: "advanced",
...
export const deviceTypesByCategory = (cat: DeviceCategory) =>
  DEVICE_TYPES.filter((d) => d.category === cat);

export const categoryLabel = (c: DeviceCategory) =>
  DEVICE_CATEGORIES.find((x) => x.code === c)?.label ?? c;
export const categoryIcon = (c: DeviceCategory) =>
  DEVICE_CATEGORIES.find((x) => x.code === c)?.icon ?? "•";
...
export function deviceInfectionRisk(
  ...
  if (patient) {
    ...
    const immune = (patient.diagnoses ?? []).some((d) =>
      /imunossupr|quimio|transplante|corticoid|HIV|neutropen/i.test(d.label),
    );
...
export const deviceSemaphore = (code: string): DeviceSemaphore | undefined =>
  DEVICE_SEMAPHORE[code];
