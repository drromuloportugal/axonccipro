  ...
  return (
    <div className={cn("border-l border-gray-500 pl-1.5 first:border-l-0 first:pl-0", className)}>
      <div className="mb-1 flex items-baseline gap-1 border-b border-gray-500 pb-1">
        ...
        <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-900">
          {title}
        </span>
      </div>
...
export function PatientPrintView({ patient }: { patient: Patient }) {
  ...
  const isGaso = (code?: string, label?: string) =>
    /pH|PaO2|PaCO2|HCO3|SatO2/i.test(code ?? label ?? "");
  ...
  return (
    <div className="bg-white p-4 text-black">
      <div className="mb-3 flex items-end justify-between border-b-[2.5px] border-black pb-2">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.28em] text-gray-600">
            PASSÔMETRO · UTI
          </div>
          <h1 className="mt-0.5 text-[19px] font-bold leading-tight tracking-tight">
            {" "}
            {patient.name}
          </h1>
          <div className="mt-0.5 text-[11px] text-gray-700">
            ...
            <span className="text-gray-400"> · </span> {patient.weight}kg
            {patient.height ? `/${patient.height}cm` : ""}
            {bmi ? (
              <>
                <span className="text-gray-400"> · </span>IMC{" "}
                <span className="font-mono">{bmi.value}</span> ({bmi.label})
              </>
            ) : null}
          </div>
          <div className="text-[10.5px] text-gray-700">
            {" "}
            UTI <span className="font-mono">D{patient.daysICU}</span>
            <span className="text-gray-400"> · </span> Hosp{" "}
            <span className="font-mono">D{patient.daysHosp}</span>
        ...
        <div className="text-right text-[10px] text-gray-700">
          <div className="inline-block rounded border border-gray-400 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
            {" "}
      ...
      <div
        ...
        style={{ gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr 1fr 1.35fr" }}
      >
        <Col title="Identificação" idx={1}>
          <ul className="space-y-0.5">
            ...
            {patient.birthDate && (
              <li>
                <b>Nasc:</b> {new Date(patient.birthDate).toLocaleDateString("pt-BR")} ({age}a)
              </li>
            )}
            <li>
              <b>Adm Hosp:</b> {patient.admissionHosp}
            </li>
            <li>
              <b>Adm UTI:</b> {patient.admissionICU}
            </li>
            <li>
              <b>Alergias:</b> {patient.allergies.join(", ") || "—"}
            </li>{" "}
            {(patient.legalRepresentative?.name || patient.legalRepresentative?.phone) && (
              <li>
                <b>Repr. legal:</b> {patient.legalRepresentative.name ?? "—"}
                {patient.legalRepresentative.relation
                  ? ` (${patient.legalRepresentative.relation})`
                  : ""}
                {patient.legalRepresentative.phone ? ` · ${patient.legalRepresentative.phone}` : ""}
              </li>
            )}
            {(patient.legalRepresentative2?.name || patient.legalRepresentative2?.phone) && (
              <li>
                <b>Repr. legal 2:</b> {patient.legalRepresentative2.name ?? "—"}
                {patient.legalRepresentative2.relation
                  ? ` (${patient.legalRepresentative2.relation})`
                  : ""}
                {patient.legalRepresentative2.phone
                  ? ` · ${patient.legalRepresentative2.phone}`
                  : ""}
              </li>
            )}
            {patient.advanceDirective &&
              (patient.advanceDirective.intubation !== "unknown" ||
                ...
                <li>
                  <b>Diretivas:</b> {directiveLabel(patient.advanceDirective.intubation, "Entubar")}{" "}
                  · {directiveLabel(patient.advanceDirective.resuscitation, "RCP")}
                  {patient.advanceDirective.notes ? ` — ${patient.advanceDirective.notes}` : ""}
                </li>
              )}
            {patient.organDonation && patient.organDonation !== "unknown" && (
              <li>
                <b>Doação de órgãos:</b> {organDonationLabel[patient.organDonation]}
              </li>
            )}
              ...
              <ul className="space-y-0.5">
                ...
                {patient.procedures.map((p, i) => (
                  <li key={i}>
                    ...
                    <span className={kindClass[p.kind]}>{p.label}</span>{" "}
        ...
        <Col title="Dispositivos invasivos" idx={3}>
          {" "}
          {activeDevices.length === 0 && (
            <div className="text-gray-500">Nenhum dispositivo ativo.</div>
          )}
                    ...
                    return (
                      <li key={d.id}>
                        ...
                        <span className="text-gray-600">
                          {" "}
                          ({formatDeviceDays(d)}/max {r.max}d)
                        </span>
                      </li>
                    );
                  })}
        ...
        <Col title="Medicações por classe" idx={4}>
          {" "}
          {MEDICATION_CLASS_ORDER.map((cls) => {
            ...
            return (
              <div key={cls} className="mb-1">
                <div className={`text-[9px] font-bold uppercase tracking-wider ${meta.className}`}>
                ...
                <ul className="ml-2 space-y-0.5">
                  ...
                  {list.map((m, i) => {
                    ...
                    return (
                      <li key={i}>
                        <span className={`font-semibold ${kindClass[m.kind]}`}>{m.name}</span>{" "}
                        {m.active === false && <span className="text-gray-500"> (susp.)</span>}
                        ...
                        {prog && (
                          <div className="text-[9px] text-gray-700">
                            ...
                            {alert && (
                              <span className="font-bold text-red-700"> · {alert.label}</span>
                            )}
                          </div>
                        )}
                      </li>
        ...
        <Col title="Culturas · Lab · Gaso · Imagem" idx={5}>
          {" "}
          {/* 1) Culturas */}
                        ...
                        <div className="mt-0.5 flex flex-wrap gap-1">
                          ...
                          {im.images.map((img) => (
                            <img
                              ...
                              className="h-16 w-16 border border-gray-300 object-cover"
                            />
                          ))}
                        </div>
                      )}
        ...
        <Col title="Estado · Vitais · BH · Bristol" idx={6}>
          <ul className="space-y-1">
            ...
            {computed.map((c, i) => (
              <li key={i}>
                <div
                  className={`text-[9px] font-bold uppercase tracking-wider ${kindClass[c.kind]}`}
                >
                  {c.sys}
                </div>
                  ...
                  {vitals.map((v) => {
                    ...
                    return (
                      <tr key={v.key} className="border-b border-gray-200">
                        ...
                        <td className={`py-0.5 text-right font-mono ${cls}`}>
                          {formatVitalValue(v)} {v.unit}
                        </td>
                        <td className="py-0.5 pl-1 text-right text-[9px] text-gray-500">
                          {levelLabel(v.level)}
                        </td>
                      </tr>
                    );
          ...
          {bristol && (
            <>
              ...
              <div>
                <b>Tipo {bristol.value}</b> — {bristol.hint}
              </div>
            </>
          )}
            ...
            (fluidBalance.totalIntake || fluidBalance.totalOutput || fluidBalance.totalDrains) && (
              <>
                ...
                <div className="font-mono">
                  {" "}
                  Entr. +{fluidBalance.totalIntake} · Saíd. −{fluidBalance.totalOutput} · Dren. −
                  {fluidBalance.totalDrains}
                </div>
                <div
                  className={`font-mono font-bold ${fluidBalance.balance < -500 ? "text-red-700" : fluidBalance.balance > 500 ? "text-amber-700" : ""}`}
                >
                  {" "}
                  BH: {fluidBalance.balance >= 0 ? "+" : ""}
                  {fluidBalance.balance} mL
                </div>{" "}
        ...
        <Col
          title="Plano · Sistemas"
          idx={7}
          className="pl-1"
          contentClassName="text-[9.5px] leading-[1.25]"
        >
          <ul className="space-y-1">
            {" "}
            {patient.conducts.map((c, i) => {
              ...
              return (
                <li key={i} className="text-justify text-[9.5px] leading-tight">
                  {meta && (
                    <span
                      className={`text-[8px] font-bold uppercase tracking-wider ${meta.className}`}
                    >
                      {meta.short}
                    </span>
                  )}{" "}
                    ...
                    visibleSubs.map((sub, si) => {
                      ...
                      return (
                        <span key={si} className={colMeta.textClass || "text-gray-900"}>
                          {sub.date && (
                            <span className="text-gray-500">{formatDayMonth(sub.date)}</span>
                          )}
                          {sub.date ? ` ${txt}` : txt}
      ...
      <div className="mt-3 border-t border-gray-500 pt-1 text-[8px] text-gray-500">
        {" "}
        Documento gerado pelo PASSÔMETRO — uso interno para passagem de plantão.
      </div>
    </div>
  );
}
