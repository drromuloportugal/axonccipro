...
export function PatientPrintView({ patient }: { patient: Patient }) {
  ...
  return (
    <div className="bg-white p-4 text-black">
      <div className="mb-3 flex items-end justify-between border-b-[2.5px] border-black pb-2">
        <div>
          ...
          <div className="text-[10.5px] text-gray-700">
            ...
            <span className="text-gray-400"> · </span>
            Hosp <span className="font-mono">D{patient.daysHosp}</span>
            <span className="text-gray-400"> · </span>
            Equipe {patient.team}
            <span className="text-gray-400"> · </span>
            {patient.attending}
      ...
      <div className="grid grid-cols-7 gap-2">
        <Col title="Identificação" idx={1}>
          <ul className="space-y-0.5">
            {patient.birthDate && <li><b>Nasc:</b> {new Date(patient.birthDate).toLocaleDateString("pt-BR")} ({age}a)</li>}
            <li><b>Adm Hosp:</b> {patient.admissionHosp}</li>
            <li><b>Adm UTI:</b> {patient.admissionICU}</li>
            <li><b>Alergias:</b> {patient.allergies.join(", ") || "—"}</li>
        ...
        <Col title="Dispositivos invasivos" idx={3}>
          {activeDevices.length === 0 && <div className="text-gray-500">Nenhum dispositivo ativo.</div>}
                  ...
                  {list.map((m, i) => {
                    ...
                    return (
                      <li key={i}>
                        ...
                        <div className="font-mono text-[9px] text-gray-700">{m.dose} · {m.route} · {m.freq}</div>
                        {m.mlPerHour !== undefined && <div className="font-mono text-[9px] text-gray-700">BIC {m.mlPerHour.toFixed(1)} mL/h</div>}
                        {prog && (
              ...
              <ul className="space-y-0.5">
                {patient.cultures!.map((c) => (
                  <li key={c.id}>
                    ...
                    <b>{c.source}</b>
                    {c.organism ? <> — <i>{c.organism}</i></> : c.result === "negativa" ? " — negativa" : " — em andamento"}
        ...
        <Col title="Estado · Vitais · BH · Bristol" idx={6}>
          ...
          {vitals.length > 0 && (
            <>
              <div className="mt-2 text-[9px] font-bold uppercase tracking-wider text-gray-700">❤️ Parâmetros vitais</div>
              <table className="w-full">
                <tbody>
                  {vitals.map((v) => {
          ...
          {patient.state.fluidBalance && (fluidBalance.totalIntake || fluidBalance.totalOutput || fluidBalance.totalDrains) && (
            <>
              ...
              <div className="font-mono">
                Entr. +{fluidBalance.totalIntake} · Saíd. −{fluidBalance.totalOutput} · Dren. −{fluidBalance.totalDrains}
              </div>
              <div className={`font-mono font-bold ${fluidBalance.balance < -500 ? "text-red-700" : fluidBalance.balance > 500 ? "text-amber-700" : ""}`}>
                BH: {fluidBalance.balance >= 0 ? "+" : ""}{fluidBalance.balance} mL
              </div>
          ...
          {lppSummary.totalActive > 0 && (
            <>
              ...
              <ul className="space-y-0.5">
                {(patient.lpp ?? []).filter((l) => !l.resolvedAt).map((l) => {
              ...
              return (
                <li key={i}>
                  <div className="flex gap-1">
                    <span>{c.done ? "☑" : "☐"}</span>
                    <span>
                      {meta && <span className={`mr-1 text-[9px] font-bold uppercase tracking-wider ${meta.className}`}>{meta.icon} {meta.short}</span>}
                      <span className="text-[9px] font-bold uppercase tracking-wider text-gray-600">{c.team} </span>
                      {c.text}
                    </span>
                  </div>
      ...
      <div className="mt-3 border-t border-gray-300 pt-1 text-[8px] text-gray-500">
