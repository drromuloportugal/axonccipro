      ...
      <DialogContent className="max-h-[92vh] max-w-xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            🩹 {editing ? "Editar lesão por pressão" : "Nova lesão por pressão"}
          </DialogTitle>
        </DialogHeader>

        {/* Stage — biggest, primary control */}
        <div>
        ...
        <div className="mt-2 grid grid-cols-[1fr_90px] gap-3">
          <div>
            ...
            <select className={inputCls} value={site} onChange={(e) => setSite(e.target.value)}>
              {(["Pelve", "Tronco", "MMII", "Cabeça", "MMSS"] as const).map((reg) => (
            ...
            <div className="mt-1 text-[10px] text-muted-foreground">
              {resolvedView === "posterior" ? "Vista posterior"
        ...
        {/* Evolutions — collapsed by default; visible on edit */}
        <button
          type="button"
...
function EvolutionAdder({ defaultStage, defaultProf, onAdd }: {
  ...
  return (
    <div className="mt-2 grid grid-cols-[80px_1fr_1fr_auto] gap-1.5">
      <select className={inputCls} value={s} onChange={(e) => setS(e.target.value as LPPStage)}>
        {STAGES.map((x) => <option key={x} value={x}>{STAGE_META[x].short}</option>)}
      </select>
