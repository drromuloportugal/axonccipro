...
export type ExamCategory =
  | "lab_hemato" | "lab_bio" | "lab_hepat" | "lab_marc" | "lab_lip"
  | "lab_endo" | "lab_panc" | "lab_urina" | "microbio" | "gaso"
  | "img_rx" | "img_usg" | "img_tc" | "img_rm" | "img_eco"
  | "img_hemod" | "img_nuclear";
...
const raw: [ExamCategory, string[]][] = [
  ["lab_hemato", ["Hemograma", "Plaquetas", "Reticulócitos", "TAP", "INR", "TTPA", "Fibrinogênio", "Dímero D"]],
  ["lab_bio", ["Glicemia", "Ureia", "Creatinina", "Sódio", "Potássio", "Magnésio", "Cálcio total", "Cálcio iônico", "Cloro", "Fósforo", "Ácido úrico"]],
  ["lab_hepat", ["TGO", "TGP", "FA", "GGT", "Bilirrubinas", "Albumina", "Proteínas totais"]],
  ["lab_marc", ["PCR", "Procalcitonina", "Ferritina", "Troponina", "CK", "CK-MB", "BNP", "Lactato"]],
  ...
  ["microbio", ["Hemocultura", "Cultura traqueal", "Cultura de ferida", "Cultura de cateter", "Escovado protegido", "Pesquisa de fungos", "Antibiograma"]],
  ...
  ["img_usg", ["USG Abdome", "USG Renal", "USG Hepática", "USG Vias biliares", "USG Doppler venoso", "USG Doppler arterial", "Ecografia pulmonar"]],
...
const GROUP_META: Record<ExamGroup, { label: string; ring: string; chip: string; dot: string }> = {
  lab: { label: "Laboratório", ring: "border-l-clinical-resp", chip: "bg-clinical-resp/10 text-clinical-resp", dot: "bg-clinical-resp" },
  img: { label: "Imagem", ring: "border-l-clinical-stable", chip: "bg-clinical-stable/10 text-clinical-stable", dot: "bg-clinical-stable" },
  micro: { label: "Microbiologia", ring: "border-l-clinical-neuro", chip: "bg-clinical-neuro/10 text-clinical-neuro", dot: "bg-clinical-neuro" },
  gaso: { label: "Gasometrias", ring: "border-l-clinical-device", chip: "bg-clinical-device/10 text-clinical-device", dot: "bg-clinical-device" },
};

// ============================================================================
// Storage model — apenas marcado/desmarcado
// ============================================================================

type Store = {
  requested: Record<string, boolean>;  // key = `${examCode}::${bed}` → true = solicitado
  preseededBeds: Record<string, boolean>; // bed → true quando já semeamos os padrões
};

const STORAGE_KEY = "passometro:exams-matrix:v2";
...
function loadStore(): Store {
  ...
  } catch { return { requested: {}, preseededBeds: {} }; }
}
...
export function ExamsMatrix({
  ...
  const beds = useMemo(
    () => patients.map((p) => ({ bed: p.bed, name: p.name, id: p.id })).sort((a, b) => a.bed.localeCompare(b.bed)),
    [patients],
  );
    ...
    for (const b of beds) {
      if (!loaded.preseededBeds[b.bed]) {
        for (const code of DEFAULT_PRESELECTED) {
          loaded.requested[`${code}::${b.bed}`] = true;
        }
  ...
  useEffect(() => { if (open) saveStore(store); }, [store, open]);
  ...
  const filteredExams = useMemo(() => {
    ...
    return EXAM_CATALOG.filter((e) => {
      ...
      if (onlyRequested) {
        const any = beds.some((b) => store.requested[`${e.code}::${b.bed}`]);
        if (!any) return false;
      }
      if (q && !e.label.toLowerCase().includes(q) && !e.section.toLowerCase().includes(q)) return false;
      return true;
    });
  ...
  const totalRequested = useMemo(
    () => Object.values(store.requested).filter(Boolean).length,
    [store.requested],
  );
  ...
  const printPDF = () => {
    const rows = EXAM_CATALOG
      .map((e) => {
        const bedsRequested = beds.filter((b) => store.requested[cellKey(e.code, b.bed)]);
    ...
    const headerCells = patientsWithSelection
      .map((b) => `<th><div class="bed">${esc(b.bed)}</div><div class="pt">${esc(b.name.split(" ")[0])}</div></th>`)
      .join("");
...
<style> @page { size: A4 landscape; margin: 10mm; }
 ...
 <button
            onClick={() => { setStore(loadStore()); toast.success("Painel atualizado"); }}
 ...
 <button
            onClick={() => {
              if (!window.confirm("Redefinir todos os leitos para os exames pré-selecionados? As solicitações extras serão removidas.")) return;
              const next: Store = { requested: {}, preseededBeds: {} };
              for (const b of beds) {
                for (const code of DEFAULT_PRESELECTED) next.requested[`${code}::${b.bed}`] = true;
 ...
 <span className="truncate text-foreground" title={e.label}>{e.label}</span>
                      ...
                      return (
 <td key={b.id} className="border-b border-l border-border bg-background p-0 text-center">
 <button
                            type="button"
                            onClick={() => toggle(e.code, b.bed)}
                            className="flex h-8 w-full items-center justify-center transition-colors hover:bg-surface-2"
                            title={checked ? "Solicitado — clique para desmarcar" : "Clique para solicitar"}
                          >
 <span
